	...
	Page.prototype = {
		...
		orderGoods: function(data) {
			...
			for(var i in data) {
				html += '<li>' +
					'<div class="img">' +
					'<a href="' + Pub.getHtmlUrl('html/user/user-order-cont.html') + '?id=' + data[i].goods_id + '"><span></span><img src="' + data[i].goods_img + '"></a>' +
					'</div>' +
					'<div class="info">' +
					'<div class="name">' +
					'<a href="' + Pub.getHtmlUrl('html/user/user-order-cont.html') + '?id=' + data[i].goods_id + '"><h4>' + data[i].goods_name + '</h4></a>' +
					'</div>' +
					'<div class="sizes">';
		...
		initEv: function() {
			...
			$(".btn-comment").on("click", function() {
				...
				var url = Pub.getHtmlUrl('html/user/user-comment.html')+'?id=' + search.id + '&gid=' + index + '&img=' + img + '&name=' + name;
				location.href = url;
			})
		}
	}
