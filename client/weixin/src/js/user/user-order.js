            ...
            if (data && data.length > 0) {
                for (var i in data) {
                    ...
                    html += '</ul></a>' +
                        ...
                        '<div class="handle">';
                    if (data[i].order_status == 100) {
                        html += '<button class="order-cancel" data-index="' + data[i].id + '">取消订单</button><button class="order-pay" data-index="' + data[i].id + '">立即支付</button><button class="order-delect" data-index="' + data[i].id + '" style="display:none;">删除订单</button>';
                    } else if (data[i].order_status == 200) {
                ...
                cloud.asked('您确定要取消订单吗', function() {
                    Pub.post('/Home/order/cancel', {
                        ...
                        if (response.error_code === 0) {
                            ...
                            $(self).hide();
                            $(self).siblings().hide();
                            $(self).siblings('.order-delect').show();
