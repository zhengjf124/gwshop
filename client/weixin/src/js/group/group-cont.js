...
(function($, window, document, undefined) {
    ...
    Page.prototype = {
        ...
        getData: function() {
            ...
            Pub.post('/Home/Goods/detail', { id: _this.params.goodsId }, function(response) {
                if (response.error_code === 0) {
                    ...
                    _this.goodsInfo(response.data, response.data.goods.limit_time, _this.infoTime);
        ...
        goodsInfo: function(data, time, callback) {
            var html = '';
            if (data) {
                $('#goods_info').empty();
                html += '<div class="name">' +
                ...
                html += '</div>' +
                    ...
                    '</div>' +
                    '</div>' +
                    '<div class="time-over" id="time">' +
                    '<h6><label class="day">0</label>天<label class="hour">00</label>时<label class="minute">00</label>分<label class="second">00</label>秒后活动结束<!--<span>(已有 1562 人参团)</span>--></h6>' +
                    '</div>';
            }
            $('#goods_info').append(html);
            callback(time);
        },
        infoTime: function(data) {
            Pub.countDown(data, "#time")
        },
        ...
        buyGoodsInfo: function(data) {
            ...
            html += '</div>' +
                ...
                '</div>' +
                '<div class="price">' +
                '<span>￥<strong id="goods_price">' + data.goods.goods_price + '</strong></span>' +
                '<del>￥' + data.goods.market_price + '</del>' +
                '</div>' +
                '</div>';
            $('#goods_buy_info').append(html);
        },
        buyGoodsSpec: function(data) {
            ...
            if (data != '') {
                ...
                $('#spec_list').parent().hide();
            }
        },
        buyChooseSpec: function() {
            var _this = this;
            $('#spec_list dl dd label').on('click', function() {
        ...
        checkData: function(){
        	var _this = this;
            var limTime = Pub.toUnix(_this.params.limitTime);
            if(limTime < _this.params.nowTime){
            	$('#btn_buy').parent().addClass('disabled').css('background','#999');
            }
        },
        initEvent: function() {
            var _this = this;
