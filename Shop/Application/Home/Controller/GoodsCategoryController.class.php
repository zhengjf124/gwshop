...
class GoodsCategoryController extends ApiController
{
    ...
    */
    ...
    /**
     * pc 商品分类
     * URI : /Home/goodsCategory/Category_pc
     *
     */

    public function Category_pc(){

        $catId = I('cat_id',0);
        if($catId==0){
            $cate = M('GoodsCategory')->where(['parent_id'=>0])->order('id')->select();
            $catId = $cate[0]['id'];
        }
        $cateList['type'] = M('GoodsCategory')->where(['parent_id'=>$catId])->field('id,type_name,parent_id')->select();
        //$cateList['type'] = M('GoodsCategory')->where(['parent_id'=>$catId])->field('id,type_name,parent_id')->select();

        //$data = ['list'=>$cateList,];
        $this->_returnData($cateList);
