...
class GoodsController extends ApiController
{
    ...
    */
    ...
    public function detail(){
        ...
        $goods = M('Goods')->field('id,cat_id,goods_name,goods_price,market_price,store_count,goods_brief,is_hot,is_best,is_new,spec_type,goods_type,end_time')
            ->where(['id' => $id] )->find();
        if($goods['goods_type']==2)
        {//限时抢购
            $end_time = M('active')->where(['type'=>'limit_goods'])->getField('end_time');
            $goods['limit_time'] = $end_time;
        }
    ...
    public function search(){
        $keyword = I("keyword");
        if(!$keyword){
            $this->_returnError('50002','请输入关键字');
        }
