...
class coupon
{
    ...
    public function registerGrantCoupon($user_id)
    {
        ...
        if ($coupon_type_info) {
            foreach ($coupon_type_info as $value) {
                $data = ['coupons_type_id' => $value['id'], 'type' => 2, 'user_id' => $user_id, 'send_time' => $now];
     ...
     *
     */
    public function checkCoupon($coupons_id)
    {
        //获取优惠券信息
        $coupon_info = M('coupons')->where(array('id' => $coupons_id))->field('id,coupons_type_id,send_time,user_id,type,use_time,order_id')->find();
        if (!$coupon_info) {
        ...
        //获取优惠券对应的分类信息
        $coupon_type_info = M('coupons_type')->where(array('id' => $coupon_info['coupons_type_id']))->field('use_start_time,use_end_time')->find();
        if (!$coupon_type_info) {
