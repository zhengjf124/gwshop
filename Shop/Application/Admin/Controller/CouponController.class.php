...
class CouponController extends BaseController
{
    ...
    public function index()
    {
        ...
        $count = M('coupons_type')->count();
        $Page = new \Think\Page($count, 10);
        $show = $Page->show();
        $lists = M('coupons_type')->order('add_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function coupon_info()
    {
        if (IS_POST) {
            ...
            if (empty($data['id'])) {
                $data['add_time'] = time();
                $row = M('coupons_type')->add($data);
            } else {
                $row = M('coupons_type')->where(array('id' => $data['id']))->save($data);
            }
            if (!$row)
                $this->error('编辑代金券失败');
        ...
        $cid = I('get.id');
        if ($cid) {
            $coupon = M('coupons_type')->where(array('id' => $cid))->find();
    ...
    */
    public function make_coupon()
    {
        ...
        $type = I('get.type');
        //查询是否存在优惠券
        $data = M('coupons_type')->where(array('id' => $cid))->find();
        ...
        if (IS_POST) {
            ...
            M('coupons_type')->where("id=$cid")->setInc('send_num', $num);
    ...
    public function ajax_get_user()
    {
        ...
        $nickname = I('nickname');
        if (!empty($nickname)) {
            $condition['nick_name'] = array('like', "%$nickname%");
        }
        $model = M('user');
        ...
        $show = $Page->show();
        $userList = $model->where($condition)->order("id desc")->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function send_coupon()
    {
        ...
        if (IS_POST) {
            ...
            $user_id = I('user_id');
            $insert = '';
            $coupon = M('coupons_type')->where("id=$cid")->find();
            ...
            $sql = "insert into __PREFIX__coupons (`coupons_type_id`,`type`,`user_id`,`send_time`) VALUES $insert";
            M()->execute($sql);
            M('coupons_type')->where("id=$cid")->setInc('send_num', $able);
    ...
    public function del_coupon()
    {
        ...
        $cid = I('get.id');
        //查询是否存在优惠券
        $row = M('coupons_type')->where(array('id' => $cid))->delete();
        if ($row) {
            //删除此类型下的优惠券
            M('coupons')->where(array('cid' => $cid))->delete();
            $this->success("删除成功");
        } else {
            $this->error("删除失败");
        }
    ...
    public function coupon_list()
    {
        ...
        $cid = I('get.id');
        //查询是否存在优惠券
        $check_coupon = M('coupons_type')->field('id,type')->where(array('id' => $cid))->find();
        if (!$check_coupon['id'] > 0)
            $this->error('不存在该类型优惠券');

        //查询该优惠券的列表的数量
        $sql = "SELECT count(1) as c FROM __PREFIX__coupons  l " .
            "LEFT JOIN __PREFIX__coupons_type c ON c.id = l.coupons_type_id " . //联合优惠券表查询名称
            "LEFT JOIN __PREFIX__order o ON o.id = l.order_id " .     //联合订单表查询订单编号
            "LEFT JOIN __PREFIX__user u ON u.id = l.user_id WHERE l.coupons_type_id = " . $cid;    //联合用户表去查询用户名
        ...
        //查询该优惠券的列表
        $sql = "SELECT l.*,c.title,o.order_sn,u.nick_name FROM __PREFIX__coupons  l " .
            "LEFT JOIN __PREFIX__coupons_type c ON c.id = l.coupons_type_id " . //联合优惠券表查询名称
            "LEFT JOIN __PREFIX__order o ON o.id = l.order_id " .     //联合订单表查询订单编号
            "LEFT JOIN __PREFIX__user u ON u.id = l.user_id WHERE l.coupons_type_id = " . $cid .    //联合用户表去查询用户名
    ...
    public function coupon_list_del()
    {
        ...
        if (!$cid)
            $this->error("缺少参数值");
        //查询是否存在优惠券
        $row = M('coupons')->where(array('id' => $cid))->delete();
        if (!$row)
            $this->error('删除失败');
        $this->success('删除成功');
    }
    ...
    public function test(){
        require_once(APP_PATH . 'coupon.class.php');
        $a = new \coupon();
        //$b = $a->checkCoupon(2);
        $b = $a->getCouponList(1,100,'');
        //$b = $a->registerGrantCoupon(1);
