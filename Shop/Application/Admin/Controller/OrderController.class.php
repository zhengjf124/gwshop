...
class OrderController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        I('user_id') != 0 ? $condition['user_id'] = I('user_id') : false;//用户id

        $sort_order = I('order_by').' '.I('sort','DESC');
        ...
        $show = $Page->show();
        //获取订单列表
        $field = 'id,order_sn,user_address,consignee,mobile,add_time,order_status,pay_status,shipping_status,order_amount';
        $orderList = $orderLogic->getOrderList($condition,$field,$sort_order,$Page->firstRow,$Page->listRows);
    ...
    */
    public function ajaxdelivery(){
    	...
    	I('consignee') ? $condition['consignee'] = trim(I('consignee')) : false;
    	I('order_sn') != '' ? $condition['order_sn'] = trim(I('order_sn')) : false;
    	$condition['order_status'] = 200;//已经付款
    ...
    public function detail(){
        ...
        //$orderGoods = json_decode($order['goods'],true);
...
//        }
    ...
    public function return_info()
    {
        ...
        $user = M('user')->where("id = {$return_goods[user_id]}")->find();
        $goods = M('goods')->where("id = {$return_goods[goods_id]}")->find();
