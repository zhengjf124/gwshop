...
function getGoodsIdBySpec($spec)
{
     ...
     *
     */
    public function getHotGoods($typeId){
        $where['parent_id_path'] = array('like','0_'.$typeId.'_%');
        $typeIDs = M('GoodsCategory')->where($where)->field('id')->select();
        $cats = '';
        foreach($typeIDs as $val){
            $cats .= $val['id'].',';
        }
        $cats = trim($cats,',');
        $where2['cat_id'] = array('in',$cats);
        $goods = M('goods')->field('id,goods_name,goods_price,market_price,goods_img')->where($where2)->order('goods_sales desc')->limit(0,8)->select();
        return $goods;
    }
